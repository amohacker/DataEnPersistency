...
ALTER TABLE public.medewerkers
    ADD geslacht Char CONSTRAINT m_geslacht_chk CHECK (geslacht = 'M' or geslacht = 'V');
...
--   Detail: Failing row contains (7369, SMIT, N, TRAINER, 7902, 1985-12-17, 800.00, null, 20, J).
...
insert into medewerkers (afd, mnr, voorl, naam, chef, gbdatum, maandsal, funcie)
values (50, 8000, 'A', 'Donk', 7839, '01-01-1970', 3000, 'MANAGER');
update afdelingen
set hoofd = 8000
where anr = 50;
...
create sequence afdelingnr_seq increment by 10 start with 60 owned by public.afdelingen.anr;

insert into afdelingen (naam, locatie, anr)
values('BROOD4', 'BOTER4', nextval('afdelingnr_seq'))

ALTER TABLE public.afdelingen ALTER COLUMN anr TYPE numeric(3,0) USING anr::numeric;
...
create table adressen (
    postcode varchar(6) check (pg_catalog.char_length(postcode)=6
        AND left(postcode, 4) ~* '[0-9]{4}'
        AND right(postcode, 2)~* '[a-c]{2}'
),
huisnummer varchar,
ingangsdatum date,
einddatum date check (einddatum > ingangsdatum) not null,
telefoon text unique check (char_length(telefoon)=10 and telefoon ~ '[0-9]{10}'),
med_mnr numeric,
...
alter table medewerkers add constraint verkoper_commissie check((functie = 'VERKOPER' and comm is not null) or (functie != 'VERKOPER' and comm is null))
