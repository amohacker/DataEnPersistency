...
SELECT m.mnr
FROM medewerkers m
where m.mnr IN (SELECT i.cursist FROM inschrijvingen i WHERE i.cursus = 'JAV') AND m.mnr IN (SELECT i.cursist FROM inschrijvingen i WHERE i.cursus = 'XML');
...
SELECT m.mnr
FROM medewerkers m
WHERE m.mnr NOT IN (SELECT cursist FROM inschrijvingen WHERE cursus = 'JAV');
...
SELECT naam
FROM medewerkers m
WHERE m.mnr in (SELECT chef from medewerkers);
...
SELECT naam
FROM medewerkers m
WHERE m.mnr not in (SELECT c.chef as chef from medewerkers c where chef is not null);
...
SELECT cursus, begindatum, (SELECT count(*) FROM inschrijvingen i WHERE u.cursus = i.cursus AND u.begindatum = i.begindatum) as aantal_inschrijvingen
FROM uitvoeringen u
...
FROM medewerkers m
WHERE m.functie = 'TRAINER' AND exists
    (SELECT cursist FROM inschrijvingen i, uitvoeringen u WHERE i.cursus = u.cursus AND i.begindatum = u.begindatum AND u.docent = m.mnr AND i.cursist = m.chef AND
                                                                u.cursus IN (SELECT c.code FROM cursussen c WHERE type = 'ALG'));
...
SELECT naam
FROM medewerkers
WHERE NOT mnr IN (SELECT docent FROM uitvoeringen WHERE docent IS NOT NULL);
